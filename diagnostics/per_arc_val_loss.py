...
Run from project root:
    ...
    python -m diagnostics.per_arc_val_loss --checkpoint checkpoints/transition_v1.pt
"""
...
AROUSAL_PRIMARY = {"anxiety_to_depression", "depression_to_anxiety", "excitement_to_contentment"}
VALENCE_PRIMARY = {
...
def load_model(ckpt_path: Path, device: torch.device) -> TransitionModel:
    ckpt = torch.load(str(ckpt_path), map_location="cpu", weights_only=False)
    cfg  = ckpt.get("model_config", {"state_dim": 384, "hidden_dim": 512})
    # l2_norm key present in new checkpoints but not a constructor param — drop it
...
def main() -> None:
    ...
    emb_cache = build_embedding_cache(val_trajs, encoder, device)
